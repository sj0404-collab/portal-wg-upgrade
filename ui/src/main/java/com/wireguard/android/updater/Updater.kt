...
object Updater {
    ...
    private const val GH_LATEST = "https://api.github.com/repos/sj0404-collab/portal-wg-upgrade/releases/latest"
    private const val UPDATE_URL_FMT = "%s"
    private const val APK_NAME_PREFIX = "AetherWG-"
    ...
    private fun checkForUpdates(): Update? {
        ...
        for (i in 0 until assets.length()) {
            ...
            if (a.optString("name").endsWith(".apk")) apkUrl = a.getString("browser_download_url")
        }
        if (apkUrl.isBlank()) return null
        val ver = try { Version(tag) } catch (_: Throwable) { Version(BuildConfig.VERSION_NAME) }
        return Update(apkUrl, ver, Sha256Digest("0".repeat(64)))
    }
